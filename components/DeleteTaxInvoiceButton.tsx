...
export default function DeleteTaxInvoiceButton({ invoiceId }: { invoiceId: string }) {
  ...
  const handleDelete = async () => {
    ...
    setError("");
    try {
      const res = await fetch(`/api/tax-invoices/${invoiceId}`, { method: "DELETE" });
      if (!res.ok) {
      ...
      router.push("/accounts/tax-invoices");
      router.refresh();
    } catch {
