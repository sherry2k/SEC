...
export default function ReceiptVoucherForm({
  ...
  const [values, setValues] = useState<ReceiptVoucherFormValues>(initial);
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(false);

  const set = <K extends keyof ReceiptVoucherFormValues>(key: K, value: ReceiptVoucherFormValues[K]) =>
    setValues((prev) => ({ ...prev, [key]: value }));

  const updateItem = (key: string, patch: Partial<ReceiptVoucherItemDraft>) =>
    set("items", values.items.map((i) => (i.key === key ? { ...i, ...patch } : i)));
  const removeItem = (key: string) => set("items", values.items.filter((i) => i.key !== key));
  const addItem = () => set("items", [...values.items, emptyReceiptItem()]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const url = mode === "create" ? "/api/receipt-vouchers" : `/api/receipt-vouchers/${voucherId}`;
      ...
      const data: { error?: string; voucher?: { id: string } } = await res.json().catch(() => ({}));

      if (!res.ok) {
        setError(data.error || "Couldn't save this receipt voucher. Try again.");
        setLoading(false);
        return;
      }

      const destination = mode === "create" ? data.voucher?.id : voucherId;
      router.push(`/accounts/receipt-vouchers/${destination}`);
      router.refresh();
    } catch {
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div>
          ...
          <input value={values.toName} onChange={(e) => set("toName", e.target.value)} required className={inputClass} />
        </div>
        <div>
          <label className={labelClass}>Project</label>
                  ...
                  <tr key={item.key} className="border-b border-[var(--sec-line)] last:border-0">
                    ...
                    <td className="px-3 py-2">
                      <input
                        ...
                        placeholder="e.g. Second Payment for Consultant Fees"
                        className={smallInput}
                      />
                    </td>
                    <td className="px-3 py-2">
      ...
      >
        {loading ? <Loader2 size={18} className="animate-spin" /> : mode === "create" ? "Save voucher" : "Save changes"}
      </button>
    </form>
  );
}
